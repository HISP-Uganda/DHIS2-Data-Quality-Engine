...
function saveValidationRule(rule) {
    ...
    `);
    stmt.run(id, rule.name, rule.description || null, rule.ruleType, JSON.stringify(rule.dataElements), rule.condition || null, rule.threshold || null, rule.severity, rule.datasetId || null, rule.orgUnitLevels ? JSON.stringify(rule.orgUnitLevels) : null, rule.isActive ? 1 : 0, now, now, rule.createdBy || null);
    return {
        id,
        ...rule,
        createdAt: now,
 ...
 */
function getActiveRules(datasetId) {
    const db = (0, connection_1.getDb)();
    let query = `
 ...
 */
function validateRange(rule, dataValue, allValues) {
 ...
 */
function validateConsistency(rule, dataValue, allValues) {
    if (!rule.condition) {
        return {
            ruleId: rule.id,
            ruleName: rule.name,
    ...
    try {
        ...
        rule.dataElements.forEach((de, index) => {
            ...
            const value = allValues.find(v => v.dataElement === de &&
 ...
 */
function validateOutlier(rule, dataValue, allValues, historicalData) {
    const numValue = Number(dataValue.value);
    if (isNaN(numValue)) {
        return {
            ruleId: rule.id,
    ...
    const sameElementValues = dataSource
        .filter(v => v.dataElement === dataValue.dataElement &&
        v.orgUnit === dataValue.orgUnit &&
 ...
 */
function validateMandatory(rule, dataValue, allValues) {
    const isEmpty = dataValue.value === null ||
 ...
 */
function runValidation(dataValues, datasetId, historicalData) {
    const results = [];
    ...
    for (const rule of rules) {
        ...
        const relevantValues = dataValues.filter(dv => rule.dataElements.includes(dv.dataElement));

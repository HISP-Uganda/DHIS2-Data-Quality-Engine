...
class ConfigurationStorage {
    ...
    static save(config) {
        ...
        // Encrypt passwords
        const sourcePassEncrypted = (0, encryption_1.ensureEncrypted)(config.sourcePass);
        const destinationPassEncrypted = (0, encryption_1.ensureEncrypted)(config.destinationPass);
        const stmt = db.prepare(`
    ...
    `);
        stmt.run(id, config.name, config.description || null, config.sourceUrl, config.sourceUser, sourcePassEncrypted, config.selectedSourceDataset || null, JSON.stringify(config.selectedSourceOrgUnits || []), JSON.stringify(config.selectedSourceOrgNames || []), JSON.stringify(config.selectedDataElements || []), config.period || null, config.destinationUrl, config.destinationUser, destinationPassEncrypted, config.selectedDestDataset || null, JSON.stringify(config.selectedDestOrgUnits || []), JSON.stringify(config.selectedDestOrgNames || []), config.dataElementMapping || null, config.sourceOrgUnitTree ? JSON.stringify(config.sourceOrgUnitTree) : null, config.destinationOrgUnitTree ? JSON.stringify(config.destinationOrgUnitTree) : null, JSON.stringify(config.selectedDatasets || []), JSON.stringify(config.dataElementGroups || []), config.isActive !== false ? 1 : 0, now, now);
    ...
    static update(id, updates) {
        ...
        if (updates.sourcePass !== undefined) {
            ...
            values.push((0, encryption_1.ensureEncrypted)(updates.sourcePass));
        }
        ...
        if (updates.destinationPass !== undefined) {
            ...
            values.push((0, encryption_1.ensureEncrypted)(updates.destinationPass));
        }
     ...
     */
    static rowToConfig(row) {
        // Decrypt passwords
        const sourcePassDecrypted = (0, encryption_1.ensureDecrypted)(row.source_pass_encrypted);
        const destinationPassDecrypted = (0, encryption_1.ensureDecrypted)(row.destination_pass_encrypted);

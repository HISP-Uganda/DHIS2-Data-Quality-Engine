...
const facilityStore_1 = require("./notifications/facilityStore");
const emailService_1 = require("./notifications/emailService");
const whatsappService_1 = require("./notifications/whatsappService");
...
const comparisonQueue_1 = require("./queue/comparisonQueue");
const dhisCache_1 = require("./cache/dhisCache");
...
try {
    ...
    if ((0, connection_1.testConnection)()) {
...
app.get('/api/health', (_req, res) => {
    const dbHealth = (0, connection_1.healthCheck)();
    res.json({
...
app.post('/api/run-dq', async (req, res) => {
    ...
    try {
        ...
        const result = await (0, engine_1.runDQ)(params, onProgress);
...
app.post('/api/get-org-units', async (req, res) => {
    ...
    try {
        ...
        const orgUnitIds = rootOrgUnits.map((ou) => ou.id);
        let allOrgUnits = rootOrgUnits;
        if (orgUnitIds.length > 0) {
            const descendantsResp = await fetch(`${baseSrc}/api/organisationUnits.json?filter=path:like:[${orgUnitIds.join(';')}]&fields=id,displayName,level,parent[id],children[id,displayName,level,children::isNotEmpty]&paging=false`, { headers: { Authorization: `Basic ${authSrc}` } });
...
app.post('/api/compare-datasets', async (req, res) => {
    ...
    try {
        ...
        const targetDataset = allDatasets.find((ds) => ds.id === targetDatasetId);
        if (targetDataset) {
            datasetsToCompare.push(targetDataset);
            const otherDatasets = allDatasets.filter((ds) => ds.id !== targetDatasetId).slice(0, 2);
        ...
        console.log(`[DQ API] Comparing ${datasetsToCompare.length} datasets:`, datasetsToCompare.map((ds) => ds.displayName));
        const dataValuesByDataset = {};
        for (const dataset of datasetsToCompare) {
            try {
                console.log(`[DQ API] Fetching data values for dataset: ${dataset.displayName}`);
                const dataValuesResp = await fetch(`${baseDest}/api/dataValueSets.json?dataSet=${dataset.id}&orgUnit=${orgUnit}&period=${period}`, { headers: { Authorization: `Basic ${authDest}` } });
                if (dataValuesResp.ok) {
                    ...
                    if (dataValuesByDataset[dataset.id].length > 0) {
                        console.log(`[DQ API] Sample data values:`, dataValuesByDataset[dataset.id].slice(0, 3).map((dv) => ({
        ...
        try {
            ...
            datasetsToCompare.forEach((dataset) => {
        ...
        dataElementMap.forEach((element) => {
            const values = Array.from(element.values.entries());
        ...
        const result = {
            datasets: datasetsToCompare.map((ds) => ({
                id: ds.id,
                name: ds.displayName
            })),
            comparisonResults,
        ...
        try {
            ...
            const notificationResult = await notificationManager_1.notificationManager.sendComparisonNotifications(orgUnit, result);
...
app.post('/api/comparison-stats', (req, res) => {
    ...
    try {
        const { datasets, totalRecords, validRecords, mismatchedRecords, missingRecords, outOfRangeRecords, consensusFound } = req.body;
        (0, statsStore_1.addComparisonStats)({
            datasets,
            totalRecords,
            validRecords,
            mismatchedRecords,
...
app.get('/api/dq-runs', (_req, res) => {
    ...
    try {
        const runs = (0, statsStore_1.getAllDQRuns)();
        res.json(runs);
    }
    catch (err) {
...
app.get('/api/comparisons', (_req, res) => {
    ...
    try {
        const comparisons = (0, statsStore_1.getAllComparisons)();
...
app.post('/api/send-comparison-notifications', (req, res) => {
    ...
    const sendNotifications = async () => {
        try {
            ...
            const { notificationManager } = await Promise.resolve().then(() => __importStar(require('./notifications/notificationManager')));
            // Send notifications
...
app.get('/api/org-units/tree', async (_req, res) => {
    ...
    try {
        ...
        const response = await fetch(`${dhis2Url}/api/organisationUnits.json?fields=id,displayName,level,path,parent[id]&paging=false`, {
            headers: {
...
app.get('/api/facilities', (_req, res) => {
    ...
    try {
        const facilities = facilityStore_1.facilityStore.getAllFacilities();
        res.json(facilities);
    }
    catch (err) {
...
app.post('/api/facilities', (req, res) => {
    ...
    try {
        const facility = facilityStore_1.facilityStore.addFacility(req.body);
...
app.put('/api/facilities/:id', (req, res) => {
    ...
    try {
        const facility = facilityStore_1.facilityStore.updateFacility(req.params.id, req.body);
        if (!facility) {
...
app.delete('/api/facilities/:id', (req, res) => {
    ...
    try {
        const deleted = facilityStore_1.facilityStore.deleteFacility(req.params.id);
        if (!deleted) {
            res.status(404).json({ error: 'Facility not found' });
            return;
        }
...
app.get('/api/facilities/stats', (_req, res) => {
    ...
    try {
        const stats = facilityStore_1.facilityStore.getStats();
        res.json(stats);
    }
    catch (err) {
...
app.post('/api/notifications/configure-email', (req, res) => {
    ...
    try {
        ...
        emailService_1.emailService.configure({ host, port, secure, user, pass, from });
...
app.post('/api/notifications/configure-whatsapp', (req, res) => {
    ...
    try {
        ...
        whatsappService_1.whatsappService.configure({ accountSid, authToken, fromNumber });
...
app.post('/api/notifications/configure-sms', (req, res) => {
    ...
    try {
        const { accountSid, authToken, fromNumber } = req.body;
        smsService_1.smsService.configure({ accountSid, authToken, fromNumber });
...
app.get('/api/notifications/test-services', async (_req, res) => {
    ...
    try {
        const testResults = await notificationManager_1.notificationManager.testNotificationServices();
...
app.post('/api/notifications/test-dq', async (req, res) => {
    ...
    try {
        ...
        const notificationResult = await notificationManager_1.notificationManager.sendDQRunNotifications(orgUnitId, testResult);
...
app.post('/api/sms/send-dq-alert', async (req, res) => {
    ...
    try {
        ...
        const result = await newSmsService.sendDQAlertSMS(phone, {
            facilityName,
            period,
            totalRecords,
            validRecords,
...
app.post('/api/sms/send', async (req, res) => {
    ...
    try {
        ...
        const result = await newSmsService.sendSMS({ recipient, message }, provider || 'dhis2');
        res.json(result);
    }
    catch (err) {
...
app.get('/api/comparison-configs', (_req, res) => {
    ...
    try {
        const configs = configStorage_1.ConfigurationStorage.getAllSummaries();
        res.json(configs);
    }
    catch (err) {
...
app.get('/api/comparison-configs/:id', (req, res) => {
    ...
    try {
        const config = configStorage_1.ConfigurationStorage.getById(req.params.id);
        if (!config) {
...
app.post('/api/comparison-configs', (req, res) => {
    ...
    const { name, description, sourceUrl, sourceUser, sourcePass, selectedSourceDataset, selectedSourceOrgUnits, selectedSourceOrgNames, selectedDataElements, period, destinationUrl, destinationUser, destinationPass, selectedDestDataset, selectedDestOrgUnits, selectedDestOrgNames, dataElementMapping, sourceOrgUnitTree, destinationOrgUnitTree, selectedDatasets, dataElementGroups, isActive = true } = req.body;
    try {
        ...
        const config = configStorage_1.ConfigurationStorage.save({
            name,
            description,
            sourceUrl,
            sourceUser,
...
app.put('/api/comparison-configs/:id', (req, res) => {
    ...
    try {
        ...
        const updatedConfig = configStorage_1.ConfigurationStorage.update(req.params.id, updates);
        if (!updatedConfig) {
            res.status(404).json({ error: 'Configuration not found' });
            return;
        }
...
app.delete('/api/comparison-configs/:id', (req, res) => {
    ...
    try {
        const deleted = configStorage_1.ConfigurationStorage.delete(req.params.id);
        if (!deleted) {
            res.status(404).json({ error: 'Configuration not found' });
            return;
        }
...
app.patch('/api/comparison-configs/:id/toggle', (req, res) => {
    ...
    try {
        const config = configStorage_1.ConfigurationStorage.toggleActive(req.params.id);
        if (!config) {
            res.status(404).json({ error: 'Configuration not found' });
            return;
        }
...
app.post('/api/comparison-configs/:id/run', async (req, res) => {
    ...
    try {
        ...
        configStorage_1.ConfigurationStorage.markAsRun(req.params.id);
...
app.post('/api/cache/invalidate-instance', async (req, res) => {
    try {
        ...
        await (0, dhisCache_1.invalidateInstanceCache)(url, username, password);
...
app.post('/api/validate-auth-cached', async (req, res) => {
    try {
        const { sourceUrl, sourceUser, sourcePass } = req.body;
        const result = await (0, dhisCache_1.validateAuthCached)(sourceUrl, sourceUser, sourcePass);
...
app.post('/api/get-datasets-cached', async (req, res) => {
    try {
        const { sourceUrl, sourceUser, sourcePass } = req.body;
        const result = await (0, dhisCache_1.fetchDatasetsCached)(sourceUrl, sourceUser, sourcePass);
        res.json(result);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/get-dataset-elements-cached', async (req, res) => {
    try {
        const { sourceUrl, sourceUser, sourcePass, datasetId } = req.body;
        const result = await (0, dhisCache_1.fetchDataElementsCached)(sourceUrl, sourceUser, sourcePass, datasetId);
        res.json(result);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/queue/stats', async (_req, res) => {
    try {
        const stats = await (0, comparisonQueue_1.getQueueStats)();
        res.json(stats);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/queue/comparison', async (req, res) => {
    try {
        ...
        const jobId = await (0, comparisonQueue_1.enqueueComparison)(jobData, priority);
        if (!jobId) {
...
app.get('/api/queue/job/:jobId', async (req, res) => {
    try {
        ...
        const status = await (0, comparisonQueue_1.getComparisonJobStatus)(jobId);
        res.json(status);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.delete('/api/queue/job/:jobId', async (req, res) => {
    try {
        const { jobId } = req.params;
        const cancelled = await (0, comparisonQueue_1.cancelComparisonJob)(jobId);
        if (cancelled) {
...
app.post('/api/queue/cleanup', async (_req, res) => {
    try {
        await (0, comparisonQueue_1.cleanupOldJobs)();
...
process.on('SIGTERM', async () => {
    ...
    await (0, comparisonQueue_1.closeQueue)();
    (0, connection_1.closeDb)();
    process.exit(0);
});
...
app.listen(PORT, () => console.log(`🚀 DQ API listening on http://localhost:${PORT}`));
exports.default = app;

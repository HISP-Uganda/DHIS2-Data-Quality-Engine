...
async function runDQ(params, onProgress) {
    const { sourceUrl, sourceUser, sourcePass, dataElements, datasetDC, orgUnit, period, destinationUrl, destinationUser, destinationPass, destinationDataset: _destinationDataset, destinationOrgUnit, dataElementMapping, } = params;
    ...
    try {
        ...
        const meResp = await (0, node_fetch_1.default)(`${baseSrc}/api/me.json`, {
        ...
        const metadataResp = await (0, node_fetch_1.default)(`${baseSrc}/api/metadata.json?dataElements:filter=id:in:[${dataElements.join(';')}]&organisationUnits:filter=id:in:[${orgUnits.join(';')}]&dataSets:filter=id:in:[${datasetDC}]`, {
            headers: { Authorization: `Basic ${authSrc}` },
        });
        let metadata = { dataElements: [], organisationUnits: [], dataSets: [] };
        if (metadataResp.ok) {
        ...
        metadata.dataElements?.forEach((de) => dataElementNames.set(de.id, de.displayName));
        metadata.organisationUnits?.forEach((ou) => orgUnitNames.set(ou.id, ou.displayName));
        metadata.dataSets?.forEach((ds) => datasetNames.set(ds.id, ds.displayName));
        ...
        for (const singleOrgUnit of orgUnits) {
            ...
            if (singleDataResp.ok) {
                const singleDataValueSetsResponse = (await singleDataResp.json());
        ...
        if (allDataValues.length === 0) {
            ...
            const analyticsResp = await (0, node_fetch_1.default)(analyticsUrl.toString(), {
                headers: { Authorization: `Basic ${authSrc}` },
            });
            if (!analyticsResp.ok) {
            ...
            if (!analyticsData.rows || analyticsData.rows.length === 0) {
                const orgUnitDisplayNames = orgUnits.map(ou => orgUnitNames.get(ou) ||
            ...
            const convertedDataValues = analyticsData.rows.map((row) => ({
                dataElement: row[0],
                orgUnit: row[1],
                period: row[2],
                value: String(row[3])
        ...
        if (destinationUrl && destinationUser && destinationPass) {
            ...
            const postResp = await (0, node_fetch_1.default)(`${baseDst}/api/dataValueSets.json`, {
                method: 'POST',
                headers: {
        ...
        try {
            ...
            const notificationResult = await notificationManager_1.notificationManager.sendDQRunNotifications(orgUnit, result);
        ...
        try {
            ...
            const notificationResult = await notificationManager_1.notificationManager.sendDQRunNotifications(orgUnit, failureResult);

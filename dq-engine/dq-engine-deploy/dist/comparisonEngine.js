...
exports.performDatasetComparisonSync = performDatasetComparisonSync;
/**
 * Perform dataset comparison for a single period
 * This is the core comparison logic extracted from DataComparisonModal.tsx
 */
async function performSinglePeriodComparison(destinationUrl, destinationUser, destinationPass, orgUnit, orgUnitName, period, selectedDatasetIds, dataElementGroups, onProgress) {
    ...
    if (!orgUnitName || orgUnitName === orgUnit || orgUnitName.length === 11) {
        try {
            const ouResponse = await fetch(`${baseUrl}/api/organisationUnits/${orgUnit}.json?fields=displayName`, { headers: { Authorization: `Basic ${auth}` } });
            if (ouResponse.ok) {
    ...
    for (const datasetId of selectedDatasetIds) {
        try {
            const datasetResponse = await fetch(`${baseUrl}/api/dataSets/${datasetId}.json?fields=id,displayName,dataSetElements[dataElement[id,displayName]]`, { headers: { Authorization: `Basic ${auth}` } });
    ...
    for (let i = 0; i < selectedDatasetIds.length; i++) {
        ...
        try {
            const response = await fetch(`${baseUrl}/api/dataValueSets.json?dataSet=${datasetId}&orgUnit=${orgUnit}&period=${period}`, { headers: { Authorization: `Basic ${auth}` } });
            if (!response.ok) {
    ...
    for (let groupIndex = 0; groupIndex < dataElementGroups.length; groupIndex++) {
        ...
        for (const datasetId of selectedDatasetIds) {
            ...
            if (element) {
                ...
                const valueEntry = datasetValues.find((dv) => dv.dataElement === element.id);
 ...
 */
async function performDatasetComparisonSync(destinationUrl, destinationUser, destinationPass, orgUnit, orgUnitName, period, selectedDatasetIds, dataElementGroups, onProgress) {
    // Normalize period to array
    const periods = Array.isArray(period) ? period : [period];
    if (periods.length === 1) {
        // Single period - use existing logic directly
        return await performSinglePeriodComparison(destinationUrl, destinationUser, destinationPass, orgUnit, orgUnitName, periods[0], selectedDatasetIds, dataElementGroups, onProgress);
    }
    ...
    for (let i = 0; i < periods.length; i++) {
        ...
        const periodOnProgress = async (step, progress) => {
        ...
        try {
            const periodResult = await performSinglePeriodComparison(destinationUrl, destinationUser, destinationPass, orgUnit, orgUnitName, currentPeriod, selectedDatasetIds, dataElementGroups, periodOnProgress);

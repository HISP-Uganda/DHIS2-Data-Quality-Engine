...
exports.comparisonQueue = void 0;
exports.enqueueComparison = enqueueComparison;
exports.getComparisonJobStatus = getComparisonJobStatus;
exports.cancelComparisonJob = cancelComparisonJob;
...
exports.comparisonQueue = comparisonQueue;
try {
    exports.comparisonQueue = comparisonQueue = new bull_1.default('dataset-comparisons', REDIS_URL, {
        defaultJobOptions: {
...
if (comparisonQueue) {
    comparisonQueue.process(async (job) => {
        ...
        const { configurationId, destinationUrl, destinationUser, destinationPass, orgUnit, orgUnitName, period, selectedDatasetIds, dataElementGroups, } = job.data;
        // Report progress
        await job.progress(10);
        // Import the comparison logic (to avoid circular dependencies)
        // We'll implement this in the next step
        const { performDatasetComparisonSync } = await Promise.resolve().then(() => __importStar(require('../comparisonEngine')));
        await job.progress(20);
        // Perform the actual comparison
        const result = await performDatasetComparisonSync(destinationUrl, destinationUser, destinationPass, orgUnit, orgUnitName, period, selectedDatasetIds, dataElementGroups, async (step, progress) => {
        ...
        await (0, statsStore_1.addComparisonStats)({
 ...
 */
async function enqueueComparison(data, priority = 0) {
    if (!comparisonQueue) {
 ...
 */
async function getComparisonJobStatus(jobId) {
    if (!comparisonQueue) {
 ...
 */
async function cancelComparisonJob(jobId) {
    if (!comparisonQueue) {
        return false;
    }
    try {
